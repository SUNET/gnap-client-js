...
async function createES256ClientKeys(): Promise<ClientKeysStorage> {
  // generate key pair
  // Pre-configuration/hardcoded to use alg="ES256". Always valid?
  const alg = "ES256"; // retrievable in ellipticCurveJwk
  ...
  const random_generated_kid = generateNonce(32); // retrievable in ellipticCurveJwk
  ...
  const clientKeys: ClientKeysStorage = {
    [PRIVATE_KEY]: privateJwk,
    [PUBLIC_KEY]: publicJwk,
    [JSON_WEB_KEY]: ellipticCurveJwk,
  };
...
export async function interactionStart(
   ...
   */
  if (clientKeys) {
    setStorageClientKeys(clientKeys); // save keys created by the user software in the storage
  } else {
    try {
      // check if clientKeys exist already in storage (re-use)
      clientKeys = getStorageClientKeys();
    } catch {
      // create and save keys in storage automatically
      clientKeys = await createES256ClientKeys();
      setStorageClientKeys(clientKeys); // this function is out of createClientKeys() so that it could be reused in case user software passes keys as parameter
    }
  }
